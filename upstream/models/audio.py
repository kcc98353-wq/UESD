...
def _layer_norm(normalized_shape, eps=1e-5, elementwise_affine=True, export=False):
    try:
        return FairseqLayerNorm(
            normalized_shape,
            eps=eps,
...
class AudioEncoder(ModalitySpecificEncoder):
    ...
    def __init__(
        ...
        positional_encoder = nn.Sequential(
            ...
            *[
                nn.Sequential(
                    ...
                    SamePad(k),
                    TransposeLast(),
                    _layer_norm(embed_dim, elementwise_affine=False),
                    TransposeLast(),
                    nn.GELU(),
                )
        ...
        if modality_cfg.conv_pos_pre_ln:
            positional_encoder = nn.Sequential(_layer_norm(embed_dim), positional_encoder)
